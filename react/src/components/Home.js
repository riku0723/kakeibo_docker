...
function Home() {
  ...
  const getIncomeData = () => {
    ...
    */
    axios.get("/getIncome").then((response) => {
      if (response.status !== 200) {
        ...
        for (var ii in resultProducts) {
          incomeItems.push({
            text: resultProducts[ii].ProductName,
            amount: parseInt(resultProducts[ii].Amount),
          });
        }
  ...
  const getExpenseData = () => {
    ...
    */
    axios.get("/getExpense").then((response) => {
      if (response.status !== 200) {
        throw new Error("レスポンスエラー");
      } else {
        var resultProducts = response.data;
  ...
  const addExpense = (text, amount) => {
    ...
    axios.post("/expProduct", params).then((response) => {
      setExpenseItems([
        ...expenseItems,
        { text: inputText, amount: inputAmount, docId: docId },
      ]);
  ...
  return (
    <div className="container">
      <div className="top">
        ...
        <IncomeExpense incomeTotal={incomeTotal} expenseItems={expenseItems} />
      </div>
      <AddItem
        addIncome={addIncome}
